...
void testMatrixGraph()
{
	...
	for (const auto v : kruskal_path) std::cout << v.beginVex << " -> ";
	std::cout << kruskal_path.back().endVex << std::endl;

	std::cout << "Dijkstra:\n";
	std::vector<int> d_path;
	std::vector<int> d_short_path_table;
	mg.Dijkstra(0, d_path, d_short_path_table);
	std::cout << std::endl;

	std::cout << "Floyd:\n";
	std::vector<std::vector<int>> f_path;
	std::vector<std::vector<int>> f_short_path_table;
