...
public:
	...
	UndirectedGraph(const std::vector<VertexType>& vexs_, int numNodes_, int numEdges_)
	...
	virtual std::vector<VertexType> GetVertexTable() { return vexs; }
	...
	virtual void Prim(std::vector<VertexType>& path) = 0;
	virtual void Kruskal(std::vector<VertexType>& path) = 0;
	virtual void Dijkstra(VertexType startVex, std::vector<VertexType>& path, std::vector<EdgeType>& shortPathTable) = 0;
	virtual void Dijkstra(int startInd, std::vector<VertexType>& path, std::vector<EdgeType>& shortPathTable) = 0;
	virtual void Floyd(std::vector<std::vector<VertexType>>& path, std::vector<std::vector<EdgeType>>& shortPathTable) = 0;
	virtual void AStar(VertexType start, VertexType goal, std::vector<VertexType>& path) = 0;

private:
	virtual void validate(int ind) = 0; // 证实ind下标索引的顶点是否存在于顶点表中
	virtual void DFS(int startInd) = 0;
	virtual void BFS(int startInd) = 0;
	virtual int findTailInd(const std::vector<int>& parent, int tailInd) = 0; // 寻找末尾顶点
	virtual std::vector<VertexType> neighbors(const VertexType& cur) = 0; // 当前顶点的邻居顶点
