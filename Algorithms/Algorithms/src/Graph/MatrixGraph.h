...
public:
	MatrixGraph();
	MatrixGraph(const std::vector<VertexType>& vexs_,
		const std::vector<std::vector<EdgeType>>& arc_,
		int numNodes_, int numEdges_);
	MatrixGraph(std::vector<VertexType> vexs_, std::vector<VertexType> arc_);
...
private:
	...
	inline std::vector<VertexType> neighbors(const VertexType& cur) override; // 当前顶点的邻居顶点
	...
	inline std::vector<typename UndirectedGraph<VertexType, EdgeType>::Point> adjToCoord(); // 邻接矩阵转换为坐标矩阵

private:
	std::vector<std::vector<EdgeType>> arc; // 邻接矩阵
	std::vector<Point> coord; // 坐标矩阵
	std::vector<bool> visited; // 访问标志的数组
};
...
template <typename VertexType, typename EdgeType>
MatrixGraph<VertexType, EdgeType>::MatrixGraph(const std::vector<VertexType>& vexs_,
	const std::vector<std::vector<EdgeType>>& arc_,
	int numNodes_, int numEdges_)
	: UndirectedGraph<VertexType, EdgeType>(vexs_, numNodes_, numEdges_),
	arc(arc_), coord(std::vector<Point>(numNodes_)), visited(std::vector<bool>(numNodes_))
{
	std::cout << "Ctor(1) with vexs, arc, numNodes and numEdges." << std::endl;
}

template <typename VertexType, typename EdgeType>
MatrixGraph<VertexType, EdgeType>::MatrixGraph(std::vector<VertexType> vexs_, std::vector<VertexType> arc_)
	: UndirectedGraph<VertexType, EdgeType>(vexs_, vexs_.size(), EdgeType()),
	arc(arc_), coord(std::vector<Point>(vexs_.size())), visited(std::vector<bool>(vexs_.size()))
{
...
MatrixGraph<VertexType, EdgeType>::MatrixGraph(int numNodes_, int numEdges_)
	: UndirectedGraph<VertexType, EdgeType>(std::vector<VertexType>(numNodes_), numNodes_, numEdges_),
	arc(std::vector<std::vector<EdgeType>>(numNodes_, std::vector<EdgeType>(numNodes_))),
	coord(std::vector<Point>(numNodes_)), visited(std::vector<bool>(numNodes_))
{
...
inline void MatrixGraph<VertexType, EdgeType>::InsertVertex(VertexType v)
{
	...
	++numNodes;
	/* 更新邻接矩阵 */
	arc.push_back(std::vector<int>(numNodes, GRAPH_INFINITY)); // 在邻接矩阵最后一行后面新增一行，并用GRAPH_INFINITY初始化边值
...
template <typename VertexType, typename EdgeType>
inline std::vector<VertexType> MatrixGraph<VertexType, EdgeType>::neighbors(const VertexType& cur)
{
...
template <typename VertexType, typename EdgeType>
inline std::vector<typename UndirectedGraph<VertexType, EdgeType>::Point> MatrixGraph<VertexType, EdgeType>::adjToCoord()
{
	std::vector<Point> coordMatrix(numNodes);
	//初始化坐标矩阵的第一个元素为(0,0)
