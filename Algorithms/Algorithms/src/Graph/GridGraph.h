...
class GridGraph : public virtual Graph
{
	...
	friend void draw_grid(const GridGraph& grid, std::unordered_map<VertexInfo, int>* cost_so_far = nullptr,
		std::unordered_map<VertexInfo, VertexInfo>* came_from = nullptr, std::vector<VertexInfo>* path = nullptr,
		const VertexInfo* start = nullptr, const VertexInfo* goal = nullptr);
	...
	for (int r = 0; r < rows; r++) { // 遍历每一行
		...
		for (int c = 0; c < cols; c++) { // 遍历每一列
			...
			if (grid.walls.count(loc)) { // 判断是否是起点、终点、路径或障碍物，并用不同的颜色输出
				std::cout << RED << std::setw(2) << "#" << RESET;
			}
			else if (start != nullptr && *start == loc) {
				std::cout << GREEN << std::setw(2) << "S" << RESET;
			}
			else if (goal != nullptr && *goal == loc) {
				std::cout << GREEN << std::setw(2) << "G" << RESET;
			}
			else if (cost_so_far != nullptr && cost_so_far->count(loc))
			{
				const auto cost = (*cost_so_far)[loc];
				std::cout << BLUE << std::setw(2) << cost << RESET;
			}
			else if (path != nullptr && std::find(path->begin(), path->end(), loc) != path->end()) {
				std::cout << YELLOW << std::setw(2) << "*" << RESET;
			}
			else if (came_from != nullptr && came_from->count(loc)) {
				const VertexInfo next = (*came_from)[loc];
				if (next.x == c + 1) { std::cout << MAGENTA << std::setw(2) << " →" << RESET; }
				else if (next.x == c - 1) { std::cout << MAGENTA << std::setw(2) << " ←" << RESET; }
				else if (next.y == r + 1) { std::cout << MAGENTA << std::setw(2) << " ↓" << RESET; }
				else if (next.y == r - 1) { std::cout << MAGENTA << std::setw(2) << " ↑" << RESET; }
				else { std::cout << CYAN << std::setw(2) << "?" << RESET; }
			}
			else { // 否则就输出一个点
				std::cout << std::setw(2) << ".";
			}
